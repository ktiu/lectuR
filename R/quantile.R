...
get_z_trans <- function(mean=NULL, sd=NULL, x=NULL, z=NULL, pop=F, symbol="i") {
  ...
  if (length(c(mean, sd)) == 2) {
    result$general <- "$%s = \\frac{%s - %s}{%s}$" %>%
      sprintf(label_z, label_x, mean, sd) %>% fix_formula
    result$general_reverse <- "$%s = %s \\cdot %s + %s$" %>%
      sprintf(label_x, label_z, sd, mean) %>% fix_formula
  }
  if (length(c(mean, sd, x)) == 3) {
    ...
    result$quick_reverse <- "$%s = %s \\cdot %s + %s\\approx%s$" %>%
      sprintf(label_x, z, sd, mean, result$fmt) %>% fix_formula
  }
  return(result)
}
